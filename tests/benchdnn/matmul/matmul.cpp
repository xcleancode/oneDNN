...
dnnl_status_t init_pd(init_pd_args_t<prb_t> &init_pd_args) {
    ...
    dnnl_memory_desc_t src_d {}, wei_d {}, bia_d {};
    if (prb->sencoding != dnnl_sparse_encoding_undef) {
        if (prb->sencoding == dnnl_sparse_encoding_csr) {
            ...
            src_d = dnn_mem_t::init_md(prb->ndims, src_rt_dims.data(),
                    prb->src_dt(), prb->sencoding, nnz, dnnl_s32, dnnl_s32);
        } else {
    ...
    if (prb->wencoding != dnnl_sparse_encoding_undef) {
        if (prb->wencoding == dnnl_sparse_encoding_csr) {
            ...
            wei_d = dnn_mem_t::init_md(prb->ndims, weights_rt_dims.data(),
                    prb->wei_dt(), prb->wencoding, nnz, dnnl_s32, dnnl_s32);
        } else {
            assert(!"unsupported encoding");
        }
    } else {
...
int fill_data(data_kind_t kind, const prb_t *prb, dnn_mem_t &mem_dt,
    ...
    if ((kind == SRC && is_src_sparse) || (kind == WEI && is_wei_sparse)) {
        ...
        benchdnn_parallel_nd(nelements, [&](int64_t idx) {
...
int doit(const prb_t *prb, res_t *res) {
    ...
    if (is_src_sparse) {
        ...
        src_fp_d = dnn_mem_t::init_md(src_md.ndims, src_md.dims,
                src_md.data_type, tags[src_md.ndims]);
