...
private:
    ...
    bool is_tensor_op() const { return conf_.is_tensor_op; }
    ...
    // XXX: suboptimal.
    dim_t map_idx_src1(dim_t off) const {
        dims_t dims;
        for (int d = conf_.ndims - 1; d >= 0; --d) {
            dims[d] = off % src0_md().dims()[d];
            off /= src0_md().dims()[d];
        }

        for (int d = 0; d < conf_.ndims; ++d) {
            dims[d] *= (!conf_.broadcast_dims[d]);
        }

        return src1_md().off_v(dims);
    };
